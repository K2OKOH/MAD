...
class Faster_Rcnn(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.ins_d = Ins_D(config, config.rcnn_in_channels * config.roi_layer.out_size * config.roi_layer.out_size,
                        self.train_batch_size, 2)
        self.img_d = Img_D(config.rpn_in_channels)
    ...
    def init_tensor(self, config):
        ...
        roi_align_index_s1 = [np.array(np.ones((config.num_expected_pos_stage2 + config.num_expected_neg_stage2, 1)) * i, dtype=self.dtype) for i in range(self.train_batch_size)]
        roi_align_index_s2 = [np.array(np.ones((config.num_expected_pos_stage2 + config.num_expected_neg_stage2, 1)) * i, dtype=self.dtype) for i in range(self.train_batch_size)]

        roi_align_index_test_s1 = [np.array(np.ones((config.rpn_max_num, 1)) * i, dtype=self.dtype) for i in range(self.test_batch_size)]
        roi_align_index_test_s2 = [np.array(np.ones((config.rpn_max_num, 1)) * i, dtype=self.dtype) for i in range(self.test_batch_size)]

        self.roi_align_index_tensor_s1 = Tensor(np.concatenate(roi_align_index_s1))
    ...
    def construct(self, img_data,    img_metas,    gt_bboxes,    gt_labels,    gt_valids, \
        ...
        x_s1 = self.fpn_neck(x_s1)
        x_s2 = self.fpn_neck(x_s2)

        # add by xmj
        # image level MVC
        MV3_img_loss, img_MV_s1, img_MV_s2 = self.img_d(x_s1[0], x_s2[0])
        ...
        if self.training:
            ...
            proposal_s2, proposal_mask_s2 = self.proposal_generator(cls_score_s2, bbox_pred_s2, self.anchor_list)
        else:
            proposal_s1, proposal_mask_s1 = self.proposal_generator_test(cls_score_s1, bbox_pred_s1, self.anchor_list)
            proposal_s2, proposal_mask_s2 = self.proposal_generator_test(cls_score_s2, bbox_pred_s2, self.anchor_list)
        ...
        if self.training:
            ...
            for i in range(self.train_batch_size):
                ...
                labels_tuple_s1 += (labels_s1,)
                mask_tuple_s1 += (mask_s1,)

                bboxes_tuple_s2 += (bboxes_s2,)
                deltas_tuple_s2 += (deltas_s2,)
                labels_tuple_s2 += (labels_s2,)
                mask_tuple_s2 += (mask_s2,)

            bbox_targets_s1 = self.concat(deltas_tuple_s1)
            rcnn_labels_s1 = self.concat(labels_tuple_s1)
            bbox_targets_s1 = ops.stop_gradient(bbox_targets_s1)
            rcnn_labels_s1 = ops.stop_gradient(rcnn_labels_s1)
            rcnn_labels_s1 = self.cast(rcnn_labels_s1, ms.int32)

            bbox_targets_s2 = self.concat(deltas_tuple_s2)
            rcnn_labels_s2 = self.concat(labels_tuple_s2)
            bbox_targets_s2 = ops.stop_gradient(bbox_targets_s2)
            rcnn_labels_s2 = ops.stop_gradient(rcnn_labels_s2)
            rcnn_labels_s2 = self.cast(rcnn_labels_s2, ms.int32)
        else:
            mask_tuple_s1 += proposal_mask_s1
            bbox_targets_s1 = proposal_mask_s1
            rcnn_labels_s1 = proposal_mask_s1
            for p_i in proposal_s1:
                bboxes_tuple_s1 += (p_i[::, 0:4:1],)

            mask_tuple_s2 += proposal_mask_s2
            bbox_targets_s2 = proposal_mask_s2
            rcnn_labels_s2 = proposal_mask_s2
            for p_i in proposal_s2:
                bboxes_tuple_s2 += (p_i[::, 0:4:1],)

        if self.training:
            if self.train_batch_size > 1:
                bboxes_all_s1 = self.concat(bboxes_tuple_s1)
                bboxes_all_s2 = self.concat(bboxes_tuple_s2)
            else:
                bboxes_all_s1 = bboxes_tuple_s1[0]
                bboxes_all_s2 = bboxes_tuple_s2[0]
            rois_s1 = self.concat_1((self.roi_align_index_tensor_s1, bboxes_all_s1))
            rois_s2 = self.concat_1((self.roi_align_index_tensor_s2, bboxes_all_s2))
        else:
            ...
            if self.device_type == "Ascend":
                bboxes_all_s1 = self.cast(bboxes_all_s1, ms.float16)
                bboxes_all_s2 = self.cast(bboxes_all_s2, ms.float16)
            rois_s1 = self.concat_1((self.roi_align_index_test_tensor_s1, bboxes_all_s1))
            rois_s2 = self.concat_1((self.roi_align_index_test_tensor_s2, bboxes_all_s2))

        rois_s1 = self.cast(rois_s1, ms.float32)
        rois_s1 = ops.stop_gradient(rois_s1)
        rois_s2 = self.cast(rois_s2, ms.float32)
        rois_s2 = ops.stop_gradient(rois_s2)

        if self.training:
            roi_feats_s1 = self.roi_align(rois_s1, self.cast(x_s1[0], ms.float32), self.cast(x_s1[1], ms.float32), self.cast(x_s1[2], ms.float32), self.cast(x_s1[3], ms.float32))
            roi_feats_s2 = self.roi_align(rois_s2, self.cast(x_s2[0], ms.float32), self.cast(x_s2[1], ms.float32), self.cast(x_s2[2], ms.float32), self.cast(x_s2[3], ms.float32))
        else:
            roi_feats_s1 = self.roi_align_test(rois_s1, self.cast(x_s1[0], ms.float32), self.cast(x_s1[1], ms.float32), self.cast(x_s1[2], ms.float32), self.cast(x_s1[3], ms.float32))
            roi_feats_s2 = self.roi_align_test(rois_s2, self.cast(x_s2[0], ms.float32), self.cast(x_s2[1], ms.float32), self.cast(x_s2[2], ms.float32), self.cast(x_s2[3], ms.float32))

        roi_feats_s1 = self.cast(roi_feats_s1, self.ms_type)
        rcnn_masks_s1 = self.concat(mask_tuple_s1)
        rcnn_masks_s1 = ops.stop_gradient(rcnn_masks_s1)
        rcnn_mask_squeeze_s1 = self.squeeze(self.cast(rcnn_masks_s1, ms.bool_))

        roi_feats_s2 = self.cast(roi_feats_s2, self.ms_type)
        rcnn_masks_s2 = self.concat(mask_tuple_s2)
        rcnn_masks_s2 = ops.stop_gradient(rcnn_masks_s2)
        rcnn_mask_squeeze_s2 = self.squeeze(self.cast(rcnn_masks_s2, ms.bool_))

        # add by xmj
        # image level MVC
        MV3_ins_loss, ins_MV_s1, ins_MV_s2 = self.ins_d(roi_feats_s1, roi_feats_s2)
        # MV3_img_loss = MV3_ins_loss
        
        # add by xmj
        # cst loss
        soft_cst_s1 = self.softmax(img_MV_s1)[:,1,:,:]
        soft_cst_s1 = self.mean(soft_cst_s1)
        soft_cst_s1 = ms.numpy.tile(soft_cst_s1, ins_MV_s1.shape)
        MV_cst_loss_s1 = self.mse(ins_MV_s1, soft_cst_s1)

        soft_cst_s2 = self.softmax(img_MV_s2)[:,0,:,:]
        soft_cst_s2 = self.mean(soft_cst_s2)
        soft_cst_s2 = ms.numpy.tile(soft_cst_s2, ins_MV_s2.shape)
        MV_cst_loss_s2 = self.mse(ins_MV_s2, soft_cst_s2)
        
        MV_cst_loss = MV_cst_loss_s1 + MV_cst_loss_s2
                
        rcnn_loss_s1, rcnn_cls_loss_s1, rcnn_reg_loss_s1, _ = self.rcnn(roi_feats_s1, bbox_targets_s1, rcnn_labels_s1, rcnn_mask_squeeze_s1)
        rcnn_loss_s2, rcnn_cls_loss_s2, rcnn_reg_loss_s2, _ = self.rcnn(roi_feats_s2, bbox_targets_s2, rcnn_labels_s2, rcnn_mask_squeeze_s2)
        
        rcnn_loss = rcnn_loss_s1 + rcnn_loss_s2
        rcnn_cls_loss = rcnn_cls_loss_s1 + rcnn_cls_loss_s2
        rcnn_reg_loss = rcnn_reg_loss_s1 + rcnn_reg_loss_s2
        rpn_cls_loss = rpn_cls_loss_s1 + rpn_cls_loss_s2
        rpn_reg_loss = rpn_reg_loss_s1 + rpn_reg_loss_s2
        rpn_loss = rpn_loss_s1 + rpn_loss_s2

        output = ()
        if self.training:
            output += (rpn_loss, rcnn_loss, rpn_cls_loss, rpn_reg_loss, rcnn_cls_loss, rcnn_reg_loss, 0.1*MV3_ins_loss, 0.1*MV3_img_loss, 0.1*MV_cst_loss)
        else:
            output = self.get_det_bboxes(rcnn_cls_loss, rcnn_reg_loss, rcnn_masks_s1, bboxes_all_s1, img_metas)
